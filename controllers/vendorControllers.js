...
const updateVendors = asyncHandler(async (req, res) => {
  ...
  if (!bcrypt.compareSync(password, passwordCheck.password)) {
    return res.status(400).json({
      status: 400,
      message:
        "Your password in incorrect. Please enter the correct password to update your details",
    });
  }
...
const deleteVendor = asyncHandler(async (req, res) => {
  ...
  if (!bcrypt.compareSync(password, dbPassword)) {
    return res.status(400).json({
      status: 400,
      message:
        "Your password is incorrect. Please enter the correct password to delete your account",
    });
  }
